...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  methods: {
    highlightCode: function highlightCode() {
      ...
      var highlightedCode = this.highlightSyntax(this.code);
      codeElement.innerHTML = highlightedCode;
    },
    highlightSyntax: function highlightSyntax(code) {
      var htmlTags = ['html', 'head', 'title', 'body', 'div', 'span', 'h1', 'h2', 'h3', 'p', 'a', 'img'];
      var cssProperties = ['color', 'font-size', 'background', 'margin', 'padding', 'border'];
      var highlightedCode = code;

      // Подсветка HTML тегов
      for (var _i = 0, _htmlTags = htmlTags; _i < _htmlTags.length; _i++) {
        var tag = _htmlTags[_i];
        var tagRegex = new RegExp("&lt;/?".concat(tag, "&gt;"), 'g');
        highlightedCode = highlightedCode.replace(tagRegex, "<span class=\"tag\">&lt;".concat(tag, "&gt;</span>"));
      }

      // Подсветка CSS свойств
      for (var _i2 = 0, _cssProperties = cssProperties; _i2 < _cssProperties.length; _i2++) {
        var property = _cssProperties[_i2];
        var propertyRegex = new RegExp("(".concat(property, ":)"), 'g');
        highlightedCode = highlightedCode.replace(propertyRegex, "<span class=\"property\">".concat(property, ":</span>"));
      }
