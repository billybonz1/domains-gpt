...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  methods: {
    ...
    highlightHTML: function highlightHTML() {
      var codeElement = this.$refs.codeContainer;
      var html = codeElement.textContent;
      var tagsRegex = /(&lt;\/?)(\w+)/g;
      var attributesRegex = /(\w+)(=)/g;
      var valuesRegex = /"([^"]*)"/g;
      var highlightedHTML = html.replace(tagsRegex, '<span class="tag">$1$2</span>').replace(attributesRegex, '<span class="attribute">$1$2</span>').replace(valuesRegex, '<span class="value">"$1"</span>').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
      codeElement.innerHTML = highlightedHTML;
    },
    highlightCSS: function highlightCSS() {
      var codeElement = this.$refs.codeContainer;
      var css = codeElement.textContent;
      var selectorsRegex = /([\w-]+)(?=\s*{)/g;
      var propertiesRegex = /(\b[\w-]+\b)(?=\s*:)/g;
      var valuesRegex = /(:\s*)([^;]+)/g;
      var highlightedCSS = css.replace(selectorsRegex, '<span class="selector">$1</span>').replace(propertiesRegex, '<span class="property">$1</span>').replace(valuesRegex, '$1<span class="value">$2</span>').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
      codeElement.innerHTML = highlightedCSS;
    }
  }
});
